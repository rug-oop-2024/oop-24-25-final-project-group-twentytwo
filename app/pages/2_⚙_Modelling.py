...
from autoop.core.ml.metric import get_metric, METRICS, Metric
from autoop.core.ml.pipeline import Pipeline
from app.core.model_training_utils import train_and_evaluate_model, evaluate_models_parallel
...
input_feature_names = [feature.name for feature in features if feature.type == "numerical" or feature.type == "categorical"]
target_feature_names = [feature.name for feature in features if feature.type == "categorical" or feature.type == "numerical"]

# Selection of input and target features
selected_input_features = st.multiselect("Select input features:", input_feature_names)
selected_target_feature = st.selectbox("Select target feature:", target_feature_names)

# Determine task type based on the target feature's type
target_feature_type = next(feature.type for feature in features if feature.name == selected_target_feature)
...
selected_metric_names = st.multiselect("Choose metrics:", available_metrics)
selected_metrics = [get_metric(name) for name in selected_metric_names]

with st.popover("Open Pipeline Summary", use_container_width=True, icon="✨"):
    st.markdown("### 🗂 Selected Dataset")
    st.write(f"**{selected_dataset_name}**")

    st.markdown("### 🔢 Input Features")
    st.write(", ".join(selected_input_features) if selected_input_features else "None selected")
    ...
    st.markdown("### 📏 Metrics")
    st.write(", ".join(selected_metric_names) if selected_metric_names else "None selected")



# Initialize features for the pipeline
input_features = [feature for feature in features if feature.name in selected_input_features]
target_feature = next(feature for feature in features if feature.name == selected_target_feature)


# Train model and evaluate using Pipeline
if st.button("Train Selected Model", help="Click to start training", icon="🔥"):
...
if num_metrics == 1:
    ...
    # Only show the Metric Weights section if more than one metric is selected
    if num_metrics > 1:
        st.subheader("Metric Weights")
        metric_weights = {}
        for metric_name in selected_metric_names:
            metric_weights[metric_name] = st.number_input(f"Weight for {metric_name}", min_value=0.0, max_value=1.0, value=1.0)

        total_weight = sum(metric_weights.values())
        if total_weight > 0:
            metric_weights = {name: weight / total_weight for name, weight in metric_weights.items()}

if st.button("Evaluate All Models", help="Train and evaluate multiple models in parallel", icon="⚡"):
    if not selected_input_features:
        st.error("Please select at least one input feature.")
    elif not selected_target_feature:
        st.error("Please select a target feature.")
        ...
        with st.spinner("Training models..."):
            ...
            with st.popover("Open Suboptimal Model Metrics", use_container_width=True, icon="🤖"):
                for result in results:
